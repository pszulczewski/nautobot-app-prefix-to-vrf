...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_PREFIX_TO_VRF_xxx
namespace = Collection("prefix_to_vrf")
namespace.configure(
    {
        "prefix_to_vrf": {
            "nautobot_ver": "2.1.0",
            "project_name": "prefix-to-vrf",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.prefix_to_vrf.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.prefix_to_vrf.compose_http_timeout,
        "NAUTOBOT_VER": context.prefix_to_vrf.nautobot_ver,
        "PYTHON_VER": context.prefix_to_vrf.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.prefix_to_vrf.project_name}",
        f'--project-directory "{context.prefix_to_vrf.compose_dir}"',
    ]

    for compose_file in context.prefix_to_vrf.compose_files:
        compose_file_path = os.path.join(context.prefix_to_vrf.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.prefix_to_vrf.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.prefix_to_vrf.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations prefix_to_vrf"
...
@task(
    help={
        "version": "Version of Prefix To VRF to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml prefix_to_vrf'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'prefix_to_vrf/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.prefix_to_vrf.local):
